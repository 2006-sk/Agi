...
function TransportButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      title={label}
      className="flex size-7 shrink-0 items-center justify-center rounded-md transition-colors disabled:cursor-not-allowed focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-current"
      style={{
...
function Scrub({ player, state }: { player: MockPlayer; state: PlayerState }) {
  ...
  const nudge = useCallback(
    (e: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      else if (e.key === 'End') player.seek(PRE_DURATION_MS);
      else return;
      e.preventDefault();
    },
  ...
  return (
    <div className="relative h-6 min-w-0 flex-1">
      <div
        ...
        aria-label={
          scrubbable
            ? 'Scrub the pre-approval timeline'
            : state.segment === 'rejected'
              ? 'Rejected dispatch playback — no responder en route'
              : 'Approved dispatch playback'
        }
        aria-valuemin={0}
        ...
        style={{ color: accent }}
        className={`absolute inset-0 flex items-center focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-current ${
        ...
        DEMO_BEATS.map((beat) => {
          ...
          return (
            <button
              ...
              className="absolute top-0 h-full w-3 -translate-x-1/2 rounded-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-current"
              style={{ left: `${(beat.at / PRE_DURATION_MS) * 100}%`, color: COLOR.cyan }}
            >
              <span
...
function MockTransport({ player, state }: { player: MockPlayer; state: PlayerState }) {
  ...
  return (
    <Panel
      ...
      accent={accent}
      trailing={
        <span role="group" aria-label="Playback speed" className="flex items-center gap-1">
          {SPEEDS.map((s) => {
            ...
            return (
              <button
                ...
                className="aura-mono rounded px-1.5 py-[1px] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-current"
                style={{
                  fontSize: 9,
...
export function DemoControls({ feed }: { feed: ReturnType<typeof useAuraFeed> }) {
  ...
  return (
    <div
      className="pointer-events-auto absolute bottom-[var(--aura-gutter)] left-[var(--aura-gutter)] z-20 w-[var(--aura-panel-w)] max-w-[calc(100vw_-_2*var(--aura-gutter))] max-[1100px]:w-[248px]"
    >
      {mode === 'live' ? (
