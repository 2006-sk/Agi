      ...
      <div className="relative mt-1.5 flex items-baseline gap-2">
        ...
        {escalated && previous && (
          <motion.span
            ...
            transition={{
              duration: still ? 0 : DURATION.base,
              ease: EASE.out,
              delay: still ? 0 : 0.12,
            }}
          >
            <span className="aura-label" style={{ color: withAlpha(color, 0.8) }}>
              ↑ from
            </span>
            <span
...
function TranscriptBlock() {
    ...
    return useAuraStore.subscribe((s) => {
      ...
      for (let i = s.transcript.length - 1; i >= 0; i--) {
        ...
        setInterrupted((prev) => {
          ...
          // Drop marks whose line has aged out of the windowed transcript, so this
          // list can never outgrow the transcript over a long session.
          const live = new Set(s.transcript.map((l) => l.id));
          return [...prev.filter((m) => live.has(m)), id];
        });
        return;
      }
    });
  ...
  // Layout effect: a new line must never paint below the fold first.
  useLayoutEffect(() => {
      ...
      <AnimatePresence>
        {showJump && (
          <motion.button
            type="button"
            onClick={jumpToLive}
            className="aura-label absolute bottom-1 left-1/2 rounded-full px-2.5 py-1 outline-none focus-visible:[outline:2px_solid_var(--aura-cyan)] focus-visible:[outline-offset:3px]"
            style={{
              x: '-50%',
              color: COLOR.cyan,
