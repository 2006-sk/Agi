...
export function CityFallback() {
  // `degraded` is set by this component's own mount path, so motion here keys off
  // the operator's reduced-motion preference alone.
  ...
  return (
    <div className="relative h-full w-full">
      ...
      >
        ...
        {/* scale bar + north — this view has to be measurable, not decorative */}
        <g transform={`translate(${-HALF + 2} ${HALF - 2})`}>
          <line x1={0} y1={0} x2={BLOCK} y2={0} stroke={withAlpha(COLOR.muted, 0.8)} strokeWidth={0.4} />
          <line x1={0} y1={-1.2} x2={0} y2={1.2} stroke={withAlpha(COLOR.muted, 0.8)} strokeWidth={0.4} />
          <line x1={BLOCK} y1={-1.2} x2={BLOCK} y2={1.2} stroke={withAlpha(COLOR.muted, 0.8)} strokeWidth={0.4} />
          <text
            x={BLOCK / 2}
            y={-2.2}
            fontSize={2.1}
        ...
        <g transform={`translate(${HALF - 4} ${-HALF + 6})`}>
          <path d="M0,-4 L1.5,1 L0,0 L-1.5,1 Z" fill={withAlpha(COLOR.muted, 0.9)} />
          <text
            x={0}
            y={4.2}
            fontSize={2.3}
      ...
      <div className="pointer-events-none absolute left-1/2 top-[58px] -translate-x-1/2 text-center">
        <div className="aura-label">Grid view · 2D geometry</div>
        {address && (
          <div
            className="aura-mono mt-1"
            style={{ fontSize: 11, color: withAlpha(accent, 0.9) }}
          >
            {address}
          </div>
        )}
