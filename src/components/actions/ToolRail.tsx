...
import { useAuraStore, type StageState } from '@/state/auraStore';
import {
  TOOL_STAGES,
  TOOL_STAGE_LABEL,
  type ToolStage,
...
export function ToolRail() {
  ...
  const stages = useAuraStore((s) => s.stages);
  const doneCount = useAuraStore(
    (s) => TOOL_STAGES.filter((st) => s.stages[st] === 'done').length,
  );
  const anyActive = useAuraStore((s) =>
    TOOL_STAGES.some((st) => s.stages[st] === 'active'),
  );
  ...
  const packets = useAuraStore(
    useShallow((s) => s.tools.filter((t) => t.stage !== null).slice(-PACKET_CAP)),
  );
                ...
                return (
                  ...
                  >
                    <motion.span
                      ...
                      transition={{
                        duration: still ? 0 : tool.status === 'pending' ? 0.24 : 0.42,
                        ease: EASE.snap,
                      }}
                    />
                  </motion.span>
